...
angular.module("materialCalendar").service("materialCalendar.Calendar", [function () {
  function Calendar(year, month, options) {
    ...
    this.setWeekStartsOn = function (i) {
      ...
      if (!isNaN(d) && d >= 0 && d <= 6) {
        _this.weekStartsOn = d;
      } else {
        _this.weekStartsOn = 0;
      }

      return _this.weekStartsOn;
    };
    ...
    this.setStartDateOfMonth = function (i) {
      ...
      if (!isNaN(d) && d >= 1 && d <= 31) {
        _this.startDateOfMonth = d;
      } else {
        _this.startDateOfMonth = 1;
      }

      return _this.startDateOfMonth;
    };
    ...
    this.next = function () {
      if (_this.start.getMonth() < 11) {
        _this.init(_this.start.getFullYear(), _this.start.getMonth() + 1);

        return;
      }

      _this.init(_this.start.getFullYear() + 1, 0);
    };
    ...
    this.prev = function () {
      ...
      _this.init(_this.start.getFullYear() - 1, 11);
    }; // Month should be the javascript indexed month, 0 is January, etc.


    this.init = function (year, month) {
      var now = new Date();
      _this.year = angular.isDefined(year) ? year : now.getFullYear();
      _this.month = angular.isDefined(month) ? month : now.getMonth();
      var daysInMonth = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
      var monthLength = daysInMonth[_this.month]; // Figure out if is a leap year.
      ...
      if (angular.isDefined(_this.options.startDateOfMonth)) {
        _this.start = new Date(_this.year, _this.month, _this.startDateOfMonth);
      } else {
        _this.start = new Date(_this.year, _this.month, 1);
      }

      var date = angular.copy(_this.start);

      if (date.getDate() === 1) {
        while (date.getDay() !== _this.weekStartsOn) {
      ...
      if (_this.noOfDays !== 0) {
        while (_this.noOfDays % 7 !== 0) {
          _this.noOfDays++;
        }

        monthLength = _this.noOfDays;
      } else {
      ...
      for (var i = 0; i < monthLength; ++i) {
        ...
        _this.weeks[_this.weeks.length - 1].push(angular.copy(date)); // Increment it.
...
angular.module("materialCalendar").service("MaterialCalendarData", [function () {
  function CalendarData() {
    ...
    this.setDayContent = function (date, content) {
      _this2.data[_this2.getDayKey(date)] = content || "";
    };
  }
...
angular.module("materialCalendar").directive("calendarMd", ["$compile", "$parse", "$templateRequest", "$q", "materialCalendar.Calendar", "MaterialCalendarData", function ($compile, $parse, $templateRequest, $q, Calendar, CalendarData) {
  ...
  return {
    ...
    link: function link($scope, $element, $attrs) {
      ...
      $scope.noCache = $attrs.clearDataCacheOnLoad || false; // Parse the parent model to determine if it"s an array.
      ...
      var handleCb = function handleCb(cb, data) {
        return (cb || angular.noop)(data);
      };

      var dateFind = function dateFind(arr, date) {
        var index = -1;
      ...
      var setTemplate = function setTemplate(contents) {
      ...
      $scope.dataService = CalendarData; // Set the html contents of each date.

      var getDayKey = function getDayKey(date) {
        return $scope.dataService.getDayKey(date);
      };

      $scope.dayKey = getDayKey;

      var getDayContent = function getDayContent(date) {
        ...
        var result = (cb || angular.noop)(date); // Check for async function. This should support $http.get() and also regular $q.defer() functions.
      ...
      bootstrap(); // These are for tests, don"t remove them..
